...
def main():
 ...
 for r in feed.get('rows') or []:
  ...
  elif float(q.get('current_odd',odd))!=float(odd):
 ...
 # Small query index: priority markets first, current/open/change history only.
 items=[]
 for q in state['quotes'].values():
  ...
  items.append({'quote_id':q['quote_id'],'match':i.get('match'),'match_start':i.get('match_start'),'family':fam,'market':i.get('market'),'line':i.get('line'),'selection':i.get('selection'),'true_open':q.get('true_open_odd'),'captured_open':q.get('captured_open_odd'),'open_status':q.get('open_status'),'current':q.get('current_odd'),'current_at':q.get('current_at'),'delta_from_available_open':None if base is None else round(float(q.get('current_odd'))-float(base),4),'changes':q.get('changes',[])[-20:]})
