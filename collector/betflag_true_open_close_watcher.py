import argparse, hashlib, json, os, pathlib, re, time, unicodedata, urllib.request
...
def collect():
    ...
    for fam,slot in CORE.items():
        try:
            st,data=get(f'{BASE}/getOverviewEventsAams/0/1/0/{AGG}/{TAB_ID}/{slot}/0?channelId=0')
            if st!=200: continue
            for ev in walk(data):
...
def one_scan(state):
    ...
    rows,event_ids,errors=collect(); current={r['quote_id']:r for r in rows}; current_keys=set(current)
    ...
    if len(hist)>=2:
        ...
        try:
            ...
            continuous=(now_dt-db).total_seconds()<=90 and (db-da).total_seconds()<=90
    ...
    for k,r in current.items():
        ...
        if q is None:
            ...
            proven_absent=continuous and all(k not in s for s in prior_key_sets) and all(eid in es for es in prior_event_sets)
            status='TRUE_OPEN_RADAR_CERTIFIED_CONTINUOUS_WATCH' if proven_absent else 'FIRST_SEEN_ONLY'
    ...
    # Freeze close once kickoff has passed: last actually observed pre-kickoff price wins.
    closed=0
    ...
    hist.append({'at':now,'keys':list(current_keys),'events':list(event_ids)})
    state['recent_scans']=hist[-2:]
    state.update({'schema_version':'betflag-open-close-watch-v1','generated_at':now,'last_scan_at':now,'source_healthy':len(rows)>0,'rows_seen':len(rows),'policy':{'true_open_betflag_official':'Only with an explicit BetFlag source opening field or independently verifiable BetFlag historical proof.','true_open_radar_certified':'Promoted only when the exact quote was absent in two consecutive healthy scans <=90s apart while the fixture itself was already present, then appeared.','close':'Last observed BetFlag quote strictly before parsed kickoff.'}})
