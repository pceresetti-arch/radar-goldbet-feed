...
ROOT=pathlib.Path('feed'); NOW=datetime.now(timezone.utc).isoformat()
...
heat=load('player-heatmap-context-current.json',{'matches':[]})
...
ledger_path=ROOT/'player-context-validation-ledger.json';ledger=load('player-context-validation-ledger.json',{'schema_version':'player-context-oos-v2','entries':{}});entries=ledger.get('entries') or {}
...
for m in heat.get('matches') or []:
    ...
    for t in m.get('teams') or []:
        ...
        for p in t.get('players') or []:HM[(mmid,tid,str(p.get('player_id') or ''))]=p
...
def player_snapshot(p,hm):
    ...
    if hm:
        for k in ('heatmap_status','heatmap_offered_matches','heatmap_sample_matches','heatmap_location_samples','heatmap_centroid_x','heatmap_centroid_y','heatmap_dispersion','heatmap_dominant_zone','heatmap_final_third_share','heatmap_box_share','heatmap_central_share','heatmap_model_status'):
...
for m in ctx.get('matches') or []:
    ...
    for t in m.get('teams') or []:
        ...
        for p in t.get('players') or []:ps.append(player_snapshot(p,HM.get((mmid,tid,str(p.get('player_id') or '')))))
    ...
    if key not in entries:
        ...
        if 0<=mt<=prev_mt:entries[key]['latest_pre_kickoff_snapshot']=snap;updated+=1
if len(entries)>5000:entries=dict(sorted(entries.items(),key=lambda kv:str(kv[1].get('created_at') or ''),reverse=True)[:5000])
ledger={'schema_version':'player-context-oos-v2','updated_at':NOW,'method':'Immutable first + latest valid pre-kickoff feature snapshot per exact XI fingerprint. Includes real heatmap density when available. Outcomes joined only later.','entry_count':len(entries),'captured_this_run':captured,'updated_this_run':updated,'entries':entries}
