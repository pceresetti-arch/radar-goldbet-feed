...
HEADERS={
    ...
    'User-Agent':'Mozilla/5.0 RadarBetFlagStandardOpen/1.1'
}
...
def parse_start(v):
    try:
        return datetime.strptime(str(v),'%d-%m-%Y %H:%M').replace(tzinfo=timezone.utc)
    except Exception:
        return None
...
def family(row):
    ...
    if '1x2' in m or 'esito finale' in m or 'esito 1 x 2' in m:
        return '1X2'
    if any(x in m for x in ('under/over','over/under','under over','totale gol','totale goal','goal totali','gol totali','u/o','o/u')) or s.startswith('over') or s.startswith('under'):
        return 'TOTAL'
    return None
            ...
            if not n.startswith('(') and not sn.startswith('giocatori'):
                ...
                if rows:
                    out.append({
                        ...
                        'match_market_id':x.get('mi'),'tournament_id':x.get('ti'),'sport_id':x.get('si'),'tai':x.get('tai'),
                        'rows':rows
                    })
...
def collect_rows_recursive(x,out):
    if isinstance(x,dict):
        if x.get('mmkW') is not None:
            out.extend(classified_rows(x))
...
def enrich_detail(ev):
    ...
    try:
        ...
        ev['rows']=merged
        return ev,{'status':status,'url':url,'rows':len(rows)}
    except Exception as e:
        return ev,{'status':None,'url':url,'error':repr(e),'rows':0}
...
def main():
    ...
    # Details contain the full standard market grid (including totals). Keep calls bounded to the actionable horizon.
    candidates=[]
    for ev in events:
        st=parse_start(ev.get('start_time'))
        if st is None or (now_dt-timedelta(minutes=20) <= st <= now_dt+timedelta(hours=36)):
    ...
    state.update({'generated_at':now,'last_success_at':now,'source_status':status,'source_url':URL,'source_class':'BETFLAG_AAMS_DIRECT_STANDARD','last_seen_keys':sorted(seen_keys),'detail_calls':detail_meta})
    STATE.write_text(json.dumps(state,ensure_ascii=False,indent=2),encoding='utf-8')
    fam_counts={f:sum(1 for r in current if r.get('family')==f) for f in ('1X2','TOTAL')}
    LATEST.write_text(json.dumps({'generated_at':now,'source_status':status,'source_class':'BETFLAG_AAMS_DIRECT_STANDARD','row_count':len(current),'family_counts':fam_counts,'rows':current},ensure_ascii=False,indent=2),encoding='utf-8')
    print(json.dumps({'generated_at':now,'status':status,'events':len(events),'rows':len(current),'family_counts':fam_counts,'detail_calls':len(detail_meta),'true_open_within_scan':sum(1 for r in current if r['open_capture_status'].startswith('TRUE_OPEN'))},ensure_ascii=False,indent=2))
