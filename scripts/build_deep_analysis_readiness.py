...
for m in lineups.get('matches') or []:
    ...
    if standard_ready:
        ...
        if player_market_bet_ready:analysis_scope='FULL_WITH_PLAYER_CONTEXT'
...
payload={
    ...
    'contract':'READY requires fresh official FotMob standard XI + tactical layer synchronized to that XI + current GoldBet 1X2/Over + certified TRUE OPEN. Player props do not block standard analysis, but a player-market BET additionally requires fresh minutes/position/concession context matched to the same XI fingerprint.',
    'input_freshness_minutes':{'lineups':lineup_age,'tactical':tactical_age,'odds_movement':movement_age,'player_props':props_age,'player_context':context_age},
    'freshness_limits_minutes':{'lineups':15,'tactical':20,'odds_movement':15,'player_props':20,'player_context':30},
...
summary['ready_matches']=[{k:m.get(k) for k in ('match_market_id','match','league','start_time','minutes_to_start','readiness','analysis_scope','player_market_bet_ready','strong_drop_count')} for m in ready]
