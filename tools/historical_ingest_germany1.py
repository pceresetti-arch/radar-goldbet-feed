...
def main():
    ...
    for season, url in SOURCES:
        try:
            ...
            if len(rows) != 306:
                raise ValueError(f"expected 306 Bundesliga fixtures, got {len(rows)}")
            source_path = OUT / f"football-data-D1-{season.replace('/','-')}.csv"
    ...
    profile = {
        "schema_version": "radar-historical-germany1-source-profile-v1",
