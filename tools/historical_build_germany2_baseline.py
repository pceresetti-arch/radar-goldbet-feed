...
RAW = Path("feed/historical/germany2/football-data-DEU2.csv")
OUT = Path("feed/historical/germany2")
NORMALIZED = OUT / "2-bundesliga-2022-2026-regular.csv"
...
def main():
    ...
    selected = [r for r in source if r.get("Season") in SEASONS and r.get("League") == "2. Bundesliga"]
    appearances = {}
    ...
    for season in SEASONS:
        ...
        if len(teams) != 18 or any(accepted_appearances[t] != 34 for t in teams):
            raise ValueError(f"Germany 2 season integrity failed for {season}: teams={len(teams)}, appearances={dict(accepted_appearances)}")
    ...
    report = {
        "schema_version": "radar-historical-germany2-baseline-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_snapshot": str(RAW),
        "normalized_dataset": str(NORMALIZED),
        "selection_rule_frozen_before_evaluation": "Latest four completed source seasons 2022/2023-2025/2026; 18-team 2. Bundesliga with 34 league matches per club; post-season European playoffs excluded chronologically after the 34-match cap.",
        "seasons": SEASONS,
