...
RAW = OUT / "football-data-ENG1.csv"
...
def main():
    ...
    for season, url in SOURCES:
        try:
            ...
            if len(rows) != 380:
                raise ValueError(f"expected 380 Premier League fixtures, got {len(rows)}")
            source_path = OUT / f"football-data-E0-{season.replace('/','-')}.csv"
    ...
    profile = {
        "schema_version": "radar-historical-england1-source-profile-v1",
