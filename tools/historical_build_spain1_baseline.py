...
RAW = Path("feed/historical/spain1/football-data-ESP1.csv")
OUT = Path("feed/historical/spain1")
NORMALIZED = OUT / "laliga-2022-2026-regular.csv"
...
def main():
    ...
    selected = [r for r in source if r.get("Season") in SEASONS and r.get("League") == "LaLiga"]
    appearances = {}
    ...
    for season in SEASONS:
        ...
        if len(teams) != 20 or any(accepted_appearances[t] != 38 for t in teams):
            raise ValueError(f"Spain 1 season integrity failed for {season}: teams={len(teams)}, appearances={dict(accepted_appearances)}")
    ...
    report = {
        "schema_version": "radar-historical-spain1-baseline-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_snapshot": str(RAW),
        "normalized_dataset": str(NORMALIZED),
        "selection_rule_frozen_before_evaluation": "Latest four completed source seasons 2022/2023-2025/2026; 20-team LaLiga with 38 league matches per club; post-season European playoffs excluded chronologically after the 38-match cap.",
        "seasons": SEASONS,
