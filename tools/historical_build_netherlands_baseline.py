...
RAW = Path("feed/historical/netherlands/football-data-NLD.csv")
OUT = Path("feed/historical/netherlands")
NORMALIZED = OUT / "eredivisie-2022-2026-regular.csv"
...
def main():
    ...
    selected = [r for r in source if r.get("Season") in SEASONS and r.get("League") == "Eredivisie"]
    appearances = {}
    ...
    for season in SEASONS:
        ...
        if len(teams) != 18 or any(accepted_appearances[t] != 34 for t in teams):
            raise ValueError(f"Netherlands season integrity failed for {season}: teams={len(teams)}, appearances={dict(accepted_appearances)}")
    ...
    report = {
        "schema_version": "radar-historical-netherlands-baseline-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_snapshot": str(RAW),
        "normalized_dataset": str(NORMALIZED),
        "selection_rule_frozen_before_evaluation": "Latest four completed source seasons 2022/2023-2025/2026; 18-team Eredivisie with 34 league matches per club; post-season European playoffs excluded chronologically after the 34-match cap.",
        "seasons": SEASONS,
