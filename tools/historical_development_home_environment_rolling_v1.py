#!/usr/bin/env python3
"""Development-only rolling-origin audit for a recent league home/draw environment feature.
...
OUT = Path("feed/historical/calibration/development-home-environment-rolling-v1.json")
OUTCOMES = ["H", "D", "A"]
REPORTS = [
...
FEATURE_SETS = {
    "ELO_ONLY": ["elo_diff"],
    "ELO_PLUS_HOME_ENV": ["elo_diff", "prior_home_win_rate_120", "prior_draw_rate_120"],
}
L2 = 0.01
LEARNING_RATE = 0.08
ITERATIONS = 1800
...
def paired_uncertainty(records):
    ...
    for r in records:
        y = r["y"]
        oh = np.eye(3)[y]
        base, cand = np.array(r["ELO_ONLY"]), np.array(r["ELO_PLUS_HOME_ENV"])
        delta = (
...
def main():
    ...
    for report_path in REPORTS:
        ...
        dev.sort(key=lambda r: (r["source_date"], r.get("source_time", ""), r["home_team"], r["away_team"]))
        by_date = defaultdict(list)
        for r in dev:
            by_date[r["source_date"]].append(r)
        recent = deque(maxlen=120)
        for date in sorted(by_date):
            total = len(recent)
            home_rate = (sum(x == "H" for x in recent) + 1.0) / (total + 3.0)
            draw_rate = (sum(x == "D" for x in recent) + 1.0) / (total + 3.0)
            for r in by_date[date]:
                r["prior_home_win_rate_120"] = home_rate
                r["prior_draw_rate_120"] = draw_rate
            for r in by_date[date]:
                recent.append(r["result"])
    ...
    candidate = metrics(records, "ELO_PLUS_HOME_ENV")
    ...
    decision = "FREEZE_FOR_FUTURE_INDEPENDENT_HOLDOUT" if candidate_robust else "REJECT_CANDIDATE"
    report = {
        "schema_version": "radar-historical-development-home-environment-rolling-v1",
        ...
        "anti_hindsight": {
            ...
            "candidate_rationale": "recent league HOME and DRAW frequencies may capture pre-match environment drift not represented by a static league intercept",
            "environment_window": "last 120 completed league fixtures before the full source-date batch, Laplace-smoothed",
        ...
        "pooled": {"ELO_ONLY": base, "ELO_PLUS_HOME_ENV": candidate},
        ...
        "decision": {
            ...
            "next_gate": "A candidate frozen from development may only be judged on a new independent temporal block; the already observed final holdout remains unavailable for selection.",
        },
