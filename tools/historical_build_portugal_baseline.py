...
RAW = Path("feed/historical/portugal/football-data-PRT.csv")
OUT = Path("feed/historical/portugal")
NORMALIZED = OUT / "primeira-liga-2022-2026-regular.csv"
...
def main():
    ...
    selected = [r for r in source if r.get("Season") in SEASONS and r.get("League") == "Primeira Liga"]
    appearances = {}
    ...
    for season in SEASONS:
        ...
        for r in season_rows:
            if accepted_appearances[r["Home"]] < 34 and accepted_appearances[r["Away"]] < 34:
                regular.append(r)
                ...
                excluded.append({
                    ...
                    "reason": "POST_SEASON_PLAYOFF_AFTER_34_LEAGUE_MATCHES"
                })
        if len(teams) != 18 or any(accepted_appearances[t] != 34 for t in teams):
            raise ValueError(f"Portugal season integrity failed for {season}: teams={len(teams)}, appearances={dict(accepted_appearances)}")
    ...
    report = {
        "schema_version": "radar-historical-portugal-baseline-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_snapshot": str(RAW),
        "normalized_dataset": str(NORMALIZED),
        "selection_rule_frozen_before_evaluation": "Latest four completed source seasons 2022/2023-2025/2026; 18-team Primeira Liga with 34 league matches per club; post-season European playoffs excluded chronologically after the 34-match cap.",
        "seasons": SEASONS,
        ...
        "sample": {
            ...
            "season_match_cap_per_team": 34
        },
        "data_quality": {
            "normalized_duplicate_fixture_keys": len(normalized) - len({r["fixture_key"] for r in normalized}),
            "identity_integrity": "Each frozen season must contain exactly 18 clubs and 34 accepted league matches per club; any subsequent playoff row is excluded chronologically without using its result.",
