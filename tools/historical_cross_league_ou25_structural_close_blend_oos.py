...
OUT=ROOT/"feed/historical/markets/cross-league-ou25-structural-close-blend-oos-v1.json"
CFG=[
...
def boot(a,n=10000):
 ...
 for _ in range(n):
  ...
  fb.append(sum((x["p_blend"]-x["y"])**2-(x["p_close"]-x["y"])**2 for x in s)/N)
  fl.append(sum(ll(x["p_blend"],x["y"])-ll(x["p_close"],x["y"]) for x in s)/N)
  vb=[];vl=[]
  for __ in leagues:
   g=leagues[rng.randrange(len(leagues))];v=by[g];ss=[v[rng.randrange(len(v))] for ___ in v]
   vb.append(sum((x["p_blend"]-x["y"])**2-(x["p_close"]-x["y"])**2 for x in ss)/len(ss))
   vl.append(sum(ll(x["p_blend"],x["y"])-ll(x["p_close"],x["y"]) for x in ss)/len(ss))
 ...
 for d,batch in grouped.items():
  ...
  for r in batch:
   ...
   if rr is None:missing_identity+=1
    ...
    qo,qu=num(rr.get("AvgC>2.5")),num(rr.get("AvgC<2.5"))
    if qo is None or qu is None:missing_price+=1
    else:
     pc=(1/qo)/((1/qo)+(1/qu));y=int(int(r["home_goals"])+int(r["away_goals"])>=3)
     joined.append({"league":league,"split":r["split"],"fixture_key":r["fixture_key"],"p_struct":ps,"p_close":pc,"y":y})
   preds.append((r,h,a))
  for r,h,a in preds:
...
for i in range(21):
 w=i/20
 for x in dev:x["p_candidate"]=w*x["p_close"]+(1-w)*x["p_struct"]
 m=metric(dev,"p_candidate");grid.append({"close_weight":w,**m})
chosen=min(grid,key=lambda x:(x["log_loss"],x["brier"],-x["close_weight"]))
w=chosen["close_weight"]
for x in items:x["p_blend"]=w*x["p_close"]+(1-w)*x["p_struct"]
mc=metric(hold,"p_close");ms=metric(hold,"p_struct");mb=metric(hold,"p_blend")
by={}
for g in sorted({x["league"] for x in hold}):
 v=[x for x in hold if x["league"]==g];a=metric(v,"p_close");b=metric(v,"p_blend")
 by[g]={"n":len(v),"close":a,"blend":b,"delta_blend_minus_close":{"brier":b["brier"]-a["brier"],"log_loss":b["log_loss"]-a["log_loss"]}}
report={"schema_version":"radar-historical-cross-league-ou25-structural-close-blend-oos-v1","generated_at":datetime.utcnow().isoformat()+"Z",
 "status":"COMPLETE_DEVELOPMENT_SELECTED_FROZEN_OOS","market":"FULL_TIME_OVER_UNDER_2_5",
 "design":{"development_rows":len(dev),"holdout_rows":len(hold),"blend_formula":"w*external_average_close + (1-w)*structural_poisson",
 ...
 "selected_close_weight":w,"selected_structural_weight":1-w,"holdout_used_for_weight_selection":False,
 "structural_features":"Strictly prior-date home/away goals for and against with fixed shrinkage and date-batch updates",
 "market_probability":"De-vigged external average closing O/U 2.5 pair; not GoldBet","operational_rule_allowed":False},
 ...
 "holdout":{"external_average_close":mc,"structural_poisson":ms,"frozen_blend":mb,
 "delta_blend_minus_close":{"brier":mb["brier"]-mc["brier"],"log_loss":mb["log_loss"]-mc["log_loss"]}},
 "uncertainty":boot(hold),"by_league":by,
 "verdict":"A structural increment may be promoted only if the development-selected blend improves the frozen holdout with uncertainty and league replication. External average close remains a non-GoldBet benchmark."}
