...
RAW = Path("feed/historical/scotland1/football-data-SCO1.csv")
OUT = Path("feed/historical/scotland1")
NORMALIZED = OUT / "scottish-premiership-2022-2026-regular.csv"
...
def main():
    ...
    selected = [r for r in source if r.get("Season") in SEASONS and r.get("League") == "Scottish Premiership"]
    appearances = {}
    ...
    for season in SEASONS:
        ...
        for r in season_rows:
            if accepted_appearances[r["Home"]] < 38 and accepted_appearances[r["Away"]] < 38:
                regular.append(r)
                ...
                excluded.append({
                    ...
                    "reason": "POST_SEASON_PLAYOFF_AFTER_38_LEAGUE_MATCHES"
                })
        if len(teams) != 12 or any(accepted_appearances[t] != 38 for t in teams):
            raise ValueError(f"Scotland 1 season integrity failed for {season}: teams={len(teams)}, appearances={dict(accepted_appearances)}")
    ...
    report = {
        "schema_version": "radar-historical-scotland1-baseline-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_snapshot": str(RAW),
        "normalized_dataset": str(NORMALIZED),
        "selection_rule_frozen_before_evaluation": "Latest four completed source seasons 2022/2023-2025/2026; all 38 official Scottish Premiership matches per club, including the championship/relegation post-split phase. The 12-club split schedule is intentionally retained and is not treated as a balanced round robin.",
        "seasons": SEASONS,
        ...
        "sample": {
            ...
            "season_match_cap_per_team": 38,
            "expected_fixtures_per_season": 228,
            "post_split_phase_included": True,
            "balanced_round_robin_assumed": False
        },
        "data_quality": {
            "normalized_duplicate_fixture_keys": len(normalized) - len({r["fixture_key"] for r in normalized}),
            "identity_integrity": "Each frozen season must contain exactly 12 clubs, 228 fixtures and 38 accepted league matches per club. The post-split phase is retained; no balanced round-robin assumption is made.",
