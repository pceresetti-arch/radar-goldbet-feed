...
RAW = Path("feed/historical/denmark/football-data-DNK.csv")
OUT = Path("feed/historical/denmark")
NORMALIZED = OUT / "superliga-2022-2026-regular.csv"
...
def main():
    ...
    selected = [r for r in source if r.get("Season") in SEASONS and r.get("League") == "Superliga"]
    appearances = {}
        ...
        for r in season_rows:
            if accepted_appearances[r["Home"]] < 32 and accepted_appearances[r["Away"]] < 32:
                ...
                excluded.append({
                    ...
                    "reason": "POST_SEASON_EUROPEAN_PLAYOFF_AFTER_32_LEAGUE_MATCHES"
                })
        if len(teams) != 12 or any(accepted_appearances[t] != 32 for t in teams):
            raise ValueError(f"Denmark season integrity failed for {season}: teams={len(teams)}, appearances={dict(accepted_appearances)}")
    ...
    report = {
        "schema_version": "radar-historical-denmark-baseline-v1",
