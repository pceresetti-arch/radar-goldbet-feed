...
OUT=ROOT/"feed/historical/markets/cross-league-btts-structural-oos-v1.json"
FILES=[
...
for league,path in FILES:
 ...
 base=sum(int(r["home_goals"])>0 and int(r["away_goals"])>0 for r in dev)/len(dev)
 total_n=0;hg=ag=0.0
 ...
 for d in dates:
  ...
  for r in grouped[d]:
   ...
   p=(1-math.exp(-lh))*(1-math.exp(-la));y=int(int(r["home_goals"])>0 and int(r["away_goals"])>0)
 ...
 reports[league]={"development_rows":len(dev),"holdout_rows":len(hold),"development_btts_rate_frozen":base,
...
report={"schema_version":"radar-historical-cross-league-btts-structural-oos-v1","generated_at":datetime.utcnow().isoformat()+"Z",
 "status":"COMPLETE_FROZEN_STRUCTURAL_OOS","market":"BOTH_TEAMS_TO_SCORE",
 ...
 "verdict":"Structural BTTS probability test only. No BTTS price history is present, so fair-price, value, CLV, ROI and MMS conclusions are forbidden."}
